...
if (isset($_POST['sauv_1'])) {
    ...
    if (empty($errors)) {
        ...
        $query = $database->db_prepare("INSERT INTO missions_smur (id_rapports,id_mission,title,num_mission,date_heure,motif,bilan_cir) VALUES (?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE title = ?, num_mission = ?, date_heure = ?, motif = ?, bilan_cir = ?");
        ...
        if ($query->execute()) {
            ...
            redirect("view_mission-2-smur?id=$id&mission=$mission&sauv=ok", false);
        } else {
        ...
        redirect(`view_mission-smur?id=$id&mission=$mission`);
    }
}
        ...
        <div class="row gx-5">
            ...
            <form method="post" action="view_mission-smur?id=<?php echo $id; ?>&mission=<?php echo $mission; ?>">
                ...
                <div class="card mb-5">
                    ...
                    <div class="card-body">
                        <textarea class="form-control" id="autosize" name="editor" placeholder="Essayer de donner la meilleur image de la situation à l'arrivée" style="min-height: 100px"><?php if($exist_mission){echo str_replace("\\r\\n" ,PHP_EOL ,$find_mission['bilan_cir']);}?></textarea>
                    </div>
                </div>
                </form>
            </div>
