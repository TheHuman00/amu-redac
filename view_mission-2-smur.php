...
if (isset($_POST['sauv_1'])) {
    ...
    if (empty($errors)) {
        ...
        $query = $database->db_prepare("INSERT INTO missions_smur (id_rapports,id_mission,sexe,age,antcd,sss,EPADONO,position,airways_ql,breathing_ql,circulation_ql) VALUES (?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE sexe = ?, age = ?, antcd = ?, sss = ?, EPADONO = ?, position = ?, airways_ql = ?, breathing_ql = ?, circulation_ql = ?");
        ...
        if ($query->execute()) {
            ...
            redirect("view_mission-3-smur?id=$id&mission=$mission&sauv=ok", false);
        } else {
        ...
        redirect(`view_mission-2-smur?id=$id&mission=$mission`);
    }
}
    ...
    <div class="container">
        <div class="row gx-5">
            ...
            <div class="col-lg-8 col-xl-9">
                <form method="post" action="view_mission-2-smur?id=<?php echo $id; ?>&mission=<?php echo $mission; ?>">
                ...
                <div class="card mb-5">
                    ...
                    <div class="card-body">
                            <div class="form-floating mb-3">
                                <input class="form-control" <?php if($exist_mission && !empty($find_mission['sexe'])){echo "value=\"".base64_decode($find_mission['sexe'])."\"";}?> name="sexe" type="text" placeholder="Sexe de la victime" />
                                <label for="titre">Sexe</label>
                            </div>
                            <div class="form-floating mb-3">
                                <input class="form-control" <?php if($exist_mission && !empty($find_mission['age'])){echo "value=\"".$find_mission['age']."\"";}?> name="age" type="text" placeholder="Age" />
                                <label for="num_mission">Age</label>
                            </div>
                            <div class="form-floating mb-3">
                                <textarea class="form-control" id="autosize" name="antcd" style="min-height: 100px" placeholder="Effectuer une liste simple des antécédents connu surplace de la victime"><?php if($exist_mission && !empty($find_mission['antcd'])){echo base64_decode($find_mission['antcd']);}?></textarea>
                ...
                <div class="card mb-5">
                    ...
                    <div class="card-body">
                            <textarea class="form-control" id="autosize" name="editor" placeholder="Comment était la sécurité sur place ? Quel sont les dangers potentiel ?" style="min-height: 100px"><?php if($exist_mission && !empty($find_mission['SSS'])){echo stripslashes(str_replace("\\r\\n" ,PHP_EOL ,$find_mission['SSS']));}?></textarea>
                    </div>
                </div>
                <div class="card mb-5">
                    ...
                    <h4>EPaDoNo</h4>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="EPADONO" id="inlineRadio1" value="eveil" <?php if($exist_mission && !empty($find_mission['EPADONO'])){if($find_mission['EPADONO'] == "eveil"){echo "checked";}}?>>
                                <label class="form-check-label" for="inlineRadio1">Eveil</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="EPADONO" id="inlineRadio2" value="parole" <?php if($exist_mission && !empty($find_mission['EPADONO'])){if($find_mission['EPADONO'] == "parole"){echo "checked";}}?>>
                                <label class="form-check-label" for="inlineRadio2">Parole</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="EPADONO" id="inlineRadio3" value="douleur" <?php if($exist_mission && !empty($find_mission['EPADONO'])){if($find_mission['EPADONO'] == "douleur"){echo "checked";}}?>>
                                <label class="form-check-label" for="inlineRadio3">Douleur</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="EPADONO" id="inlineRadio4" value="no_response" <?php if($exist_mission && !empty($find_mission['EPADONO'])){if($find_mission['EPADONO'] == "no_response"){echo "checked";}}?>>
                                <label class="form-check-label" for="inlineRadio4">No response</label>
                            </div>
                            <div class="form-floating mb-3">
                                <input class="form-control" <?php if($exist_mission && !empty($find_mission['position'])){echo "value=\"".$find_mission['position']."\"";}?> name="position" type="text" placeholder="Position de la victime" />
                                <label for="titre">Position de la victime</label>
                            </div>
                            <h4>Airways (du quicklook)</h4>
                                <textarea name="editor1" class="form-control" placeholder="[Brievement] Comment était les voies respiratoires ?" id="autosize" style="min-height: 100px"><?php if($exist_mission && !empty($find_mission['airways_ql'])){echo stripslashes(str_replace("\\r\\n" ,PHP_EOL ,$find_mission['airways_ql']));}?></textarea>
                            <h4>Breathing (du quicklook)</h4>
                                <textarea name="editor2" class="form-control" placeholder="[Brievement] Comment était la respiration subjectivement ? (difficulté respi apparante ?)" id="autosize" style="min-height: 100px"><?php if($exist_mission && !empty($find_mission['breathing_ql'])){echo stripslashes(str_replace("\\r\\n" ,PHP_EOL ,$find_mission['breathing_ql']));}?></textarea>
                            <h4>Circulation (du quicklook)</h4>
                                <textarea name="editor3" class="form-control" placeholder="[Brievement] Comment était le pouls ? (Frappé, régulier ?)" id="autosize" style="min-height: 100px"><?php if($exist_mission && !empty($find_mission['circulation_ql'])){echo stripslashes(str_replace("\\r\\n" ,PHP_EOL ,$find_mission['circulation_ql']));}?></textarea>
